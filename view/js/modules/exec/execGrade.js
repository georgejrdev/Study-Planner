async function handleCreateGrade(){
    ...
    const response = await window.api.db.grade.create(subjectId, grade, origin)
    showMessage(response)
}

async function handleDeleteGrade(id){
    const response = await window.api.db.grade.delete(id)
    showMessage(response)
}
...
async function handleUpdateGrade(id){
    ...
    const response = await window.api.db.grade.update(id, grade, origin)
    showMessage(response)
}
...
function showMessage(responseObject) {
    ...
    const message = gradeMessageCode[responseObject.frontendMessageCode]
