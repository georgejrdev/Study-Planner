...
async function handleAbsence(create, date){
    if (!create){
        const response = await window.api.db.absence.delete(date)
...
async function handleCreateAbsence(){
    ...
    const response = await window.api.db.absence.create(date, reason)
    showMessage(response)
}
...
function showMessage(responseObject) {
    ...
    const message = absenceMessageCode[responseObject.frontendMessageCode]
